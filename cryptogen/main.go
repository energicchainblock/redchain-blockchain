...
var (
	...
	rootCaFile = gen.Flag("rootca", "The root ca file to use").File()
...
func main() {
	...
	case gen.FullCommand():
		gRootCerts, _ = ioutil.ReadAll(*rootCaFile)
		generate()
		//fmt.Printf("rootcafile=%v\r\n", rootCaFile)
...
func generatePeerOrg(baseDir string, orgSpec OrgSpec) {
	...
	signCA, err := ca.NewCAFromBytes(caDir, orgName, orgSpec.CA.CommonName, gRootCerts)
	if err != nil {
	...
	tlsCA, err := ca.NewCAFromBytes(tlsCADir, orgName, "tls"+orgSpec.CA.CommonName, gRootCerts)
	if err != nil {
